...
let%expect_test "identity function and its type" =
  p (Lam ("A", Sort 1, Lam ("x", Var 0, Var 0)));
  [%expect {| fun (A : Type) => fun (x : A) => x |}];
  p (Pi ("A", Sort 1, Pi ("", Var 0, Var 1)));
  [%expect {| (A : Type) -> A -> A |}]
